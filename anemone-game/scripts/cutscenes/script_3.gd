...
func _ready() -> void:
	...
	other.start()
	AudioManager.play_dialogue("vocal_6_001").finished.connect((func():
		_show_price()
		AudioManager.play_dialogue("vocal_6_002").finished.connect((func():
			other.stop()
			ane.start()
			AudioManager.play_dialogue("vocal_6_003").finished.connect((func():
				ane.stop()
